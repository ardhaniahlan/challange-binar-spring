...
import com.challangebinarspring.binarfud.request.GoogleReq;
...
//        Map<String, String> map = parameters.toSingleValueMap();
        ...
        try {
            ...
            return new ResponseEntity<Map>(response.errorResponse(e.getDetails()), HttpStatus.BAD_GATEWAY);
        }
        ...
        if (null != user) {
            if (!user.isEnabled()) {
                return new ResponseEntity<Map>(response.errorResponse("Your Account is disable. Please chek your email for activation."), HttpStatus.OK);
            }
...
//            register : tidak digunakan
            return new ResponseEntity<Map>(response.errorResponse("Username is not registered yet. Please contact admin."), HttpStatus.OK);
        }
        return new ResponseEntity<Map>(response.successResponse(map123), HttpStatus.OK);
    }
